...
def draw_cam(img_path, save_path, transform=None, visheadmap=False):
    ...
    checkpoint = torch.load('./checkpoint/inception_v3.net')

    # checkpoint = torch.load('./checkpoint/resnet.net')
    model.load_state_dict(checkpoint['net'])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    x = img.to(device)
    x = model.module.Conv2d_1a_3x3(x)
    x = model.module.Conv2d_2a_3x3(x)
    x = model.module.Conv2d_2b_3x3(x)
    x = model.module.maxpool1(x)

    x = model.module.Conv2d_3b_1x1(x)
    x = model.module.Conv2d_4a_3x3(x)
    x = model.module.maxpool2(x)
    x = model.module.Mixed_5b(x)
    x = model.module.Mixed_5c(x)
    x = model.module.Mixed_5d(x)
    x = model.module.Mixed_6a(x)
    x = model.module.Mixed_6b(x)
    x = model.module.Mixed_6c(x)
    x = model.module.Mixed_6d(x)
    x = model.module.Mixed_6e(x)
    x = model.module.Mixed_7a(x)
    x = model.module.Mixed_7b(x)
    x = model.module.Mixed_7c(x)
...
if __name__ == '__main__':
    ...
    draw_cam('data/NTM_test/person35_1.png', 'data/NTM_test/cam0004.png', transform=transform, visheadmap=True)
